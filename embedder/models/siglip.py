import numpy as np
import torch
from PIL import Image
from .base import BaseEmbedder


_VARIANTS = {
    "siglip-base-patch16-224":    ("google/siglip-base-patch16-224",    768),
    "siglip-large-patch16-256":   ("google/siglip-large-patch16-256",   1024),
    "siglip-so400m-patch14-384":  ("google/siglip-so400m-patch14-384",  1152),
    "siglip2-so400m-patch16-384": ("google/siglip2-so400m-patch16-384", 1152),
}


class SigLIPEmbedder(BaseEmbedder):
    def __init__(self, variant: str = "siglip-so400m-patch14-384", device: str = "auto", batch_size: int = 32):
        super().__init__(device)
        if variant not in _VARIANTS:
            raise ValueError(f"Unknown variant '{variant}'. Choose from: {list(_VARIANTS)}")
        self._variant = variant
        self._hf_id, self._dim = _VARIANTS[variant]
        self.batch_size = batch_size
        self._model = None
        self._processor = None

    def load(self) -> None:
        from transformers import SiglipImageProcessor, SiglipVisionModel
        self._processor = SiglipImageProcessor.from_pretrained(self._hf_id)
        self._model = SiglipVisionModel.from_pretrained(self._hf_id).to(self.device).eval()

    def embed(self, images: list[Image.Image]) -> np.ndarray:
        if self._model is None:
            self.load()
        all_embeddings = []
        for i in range(0, len(images), self.batch_size):
            batch = images[i : i + self.batch_size]
            inputs = self._processor(images=batch, return_tensors="pt").to(self.device)
            with torch.no_grad():
                emb = self._model(**inputs).pooler_output
            emb = emb / emb.norm(dim=-1, keepdim=True)
            all_embeddings.append(emb.cpu().numpy())
        return np.concatenate(all_embeddings, axis=0)

    @property
    def embedding_dim(self) -> int:
        return self._dim

    @property
    def model_name(self) -> str:
        return self._variant
